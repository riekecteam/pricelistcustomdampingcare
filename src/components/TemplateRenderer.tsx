...
export function TemplateRenderer({ data, template, width, height }: Props) {
  ...
  const priceStyle: React.CSSProperties = { ...contentStyle, fontSize: `${data.priceSize * scale}px`, color: data.priceColor, fontWeight: 700 };
  const common = { data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services };
...
function Logo({ data, scale, rounded = true }: { data: PriceListData; scale: number; rounded?: boolean }) {
  ...
  if (!data.logo) {
    return (
      ...
      >
        <span style={{ fontSize: sz * 0.4 }} className="text-white font-bold">{(data.businessName || 'D')[0]}</span>
      </div>
    );
  }
  return <img src={data.logo} alt="logo" style={{ width: sz, height: sz, objectFit: 'contain' }} className={rounded ? 'rounded-2xl' : ''} />;
}

// T1: Minimal Clean
function T1Minimal({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full bg-white flex flex-col items-center px-[8%] py-[6%]">
      <Logo data={data} scale={scale} />
      {data.businessName && <p style={{ fontFamily: font, fontSize: 16 * scale, color: data.contentColor }} className=" mt-3">{data.businessName}</p>}
      <Icon className="text-primary mt-4" strokeWidth={1.5} style={{ width: 40 * scale, height: 40 * scale }} />
      <h1 style={titleStyle}>{data.title}</h1>
      {data.subtitle && <p style={contentStyle}>{data.subtitle}</p>}
      <div className="w-full mt-6 space-y-3">
        {services.map((s) => (
          <div key={s.id} className="flex items-baseline justify-between border-b border-lavender-100 pb-2">
            <div className="flex-1">
              <p style={{...contentStyle, color: data.titleColor}} className="font-semibold">{s.name}</p>
              {s.description && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor, opacity: 0.7 }}>{s.description}</p>}
            </div>
            <p style={priceStyle} className="font-bold ml-2">{s.price}</p>
          </div>
        ))}
      </div>
      {data.serviceArea && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-4">📍 {data.serviceArea}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3 text-center">{data.notes}</p>}
    </div>
  );
}

// T2: Soft Pink
function T2SoftPink({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col items-center px-[8%] py-[6%]" style={{ background: 'linear-gradient(180deg, #FFF0F7 0%, #FFE4F0 100%)' }}>
      ...
      {data.subtitle && <p style={contentStyle}>{data.subtitle}</p>}
      {data.businessName && <p style={{ fontFamily: font, fontSize: 14 * scale, color: data.contentColor }} className=" mt-1">{data.businessName}</p>}
      <div className="w-full mt-6 space-y-3">
        {services.map(s => (
...
function T3LuxuryWhite({ data, font, Icon, titleStyle, contentStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full bg-white flex flex-col items-center px-[10%] py-[8%]" style={{ border: '2px solid #CBB8FF' }}>
      ...
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-4">{data.businessName}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3 text-center">{data.notes}</p>}
    </div>
  );
}
...
// T5: Modern Card
function T5ModernCard({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col px-[8%] py-[6%]" style={{ background: 'linear-gradient(180deg, #F5F0FF 0%, #FFFFFF 100%)' }}>
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 style={titleStyle}>{data.title}</h1>
          {data.businessName && <p style={{ fontFamily: font, fontSize: 14 * scale, color: data.contentColor, opacity: 0.7 }}>{data.businessName}</p>}
        </div>
      </div>
      {data.subtitle && <p style={contentStyle}>{data.subtitle}</p>}
        ...
        {services.map(s => (
          <div key={s.id} className="bg-white rounded-2xl p-4 flex items-center justify-between" style={{ boxShadow: '0 4px 16px rgba(203,184,255,0.2)' }}>
            ...
            <div className="text-right">
              <p style={priceStyle} className="font-bold">{s.price}</p>
            </div>
          </div>
        ))}
      </div>
      {data.serviceArea && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-4">📍 {data.serviceArea}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3">{data.notes}</p>}
    </div>
  );
}

// T6: Korean Style
function T6Korean({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
        ...
        {services.map((s, i) => (
          <div key={s.id} className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {s.description && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-1">{s.description}</p>}
            </div>
          </div>
        ))}
      </div>
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-6">{data.businessName}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3 text-center">{data.notes}</p>}
    </div>
  );
}
...
// T7: Pinterest Style
function T7Pinterest({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        {services.map(s => (
          <div key={s.id} className="bg-white rounded-2xl p-3" style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.06)' }}>
            <p style={{...contentStyle, color: data.titleColor}} className="font-semibold">{s.name}</p>
            {s.description && <p style={{ fontFamily: font, fontSize: 11 * scale, color: data.contentColor, opacity: 0.7 }}>{s.description}</p>}
            <p style={priceStyle} className="font-bold mt-1">{s.price}</p>
          </div>
        ))}
      </div>
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-4 text-center">{data.businessName}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-2 text-center">{data.notes}</p>}
    </div>
  );
}
  ...
  return (
    <div className="w-full h-full bg-white flex flex-col px-[10%] py-[8%]">
      <div className="flex justify-between items-start">
        <Logo data={data} scale={scale} />
        <Icon strokeWidth={1} style={{ width: 32 * scale, height: 32 * scale, color: data.titleColor }} />
      </div>
      <div className="mt-8">
        <h1 style={titleStyle}>{data.title}</h1>
      ...
      <div className="mt-8 space-y-4">
        {services.map(s => (
          <div key={s.id} className="flex items-baseline justify-between">
            ...
            <p style={{...contentStyle, color: data.titleColor}} className="font-bold">{s.price}</p>
          </div>
        ))}
      </div>
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-6">{data.businessName}</p>}
      {data.serviceArea && <p style={{ fontFamily: font, fontSize: 11 * scale, color: data.contentColor }} className=" mt-1">{data.serviceArea}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3">{data.notes}</p>}
    </div>
  );
}
...
// T9: Magazine
function T9Magazine({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col px-[8%] py-[6%]" style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F5F0FF 100%)' }}>
      ...
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-4 text-center">{data.notes}</p>}
    </div>
  );
}
...
// T11: Pastel Gradient
function T11PastelGradient({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
...
// T12: Bento Grid
function T12Bento({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col px-[6%] py-[5%]" style={{ background: '#F5F0FF' }}>
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div className="bg-white rounded-3xl p-4 col-span-1 flex flex-col items-center justify-center" style={{ minHeight: 120 * scale }}>
          ...
          {data.subtitle && <p style={{ ...contentStyle, fontSize: data.contentSize * scale * 0.8 }} className="text-center mt-2">{data.subtitle}</p>}
        </div>
      </div>
      ...
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-3 text-center">{data.businessName}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-2 text-center">{data.notes}</p>}
    </div>
  );
}
...
function T13PremiumClinic({ data, font, Icon, titleStyle, contentStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col px-[8%] py-[6%]" style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F0FBF4 100%)' }}>
      ...
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-4 text-center">{data.businessName}</p>}
      {data.serviceArea && <p style={{ fontFamily: font, fontSize: 11 * scale, color: data.contentColor }} className=" mt-1 text-center">📍 {data.serviceArea}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-3 text-center">{data.notes}</p>}
    </div>
  );
}
...
// T14: Beauty Studio
function T14BeautyStudio({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
  ...
  return (
    <div className="w-full h-full flex flex-col items-center px-[10%] py-[8%]" style={{ background: '#FFFBF7' }}>
      ...
      <div className="w-full mt-6 space-y-3">
        {services.map(s => (
          <div key={s.id} className="flex items-baseline justify-between">
            <p style={{...contentStyle, color: data.titleColor}}>{s.name}</p>
            <p style={contentStyle} className="text-primary font-semibold">{s.price}</p>
          </div>
        ))}
      </div>
      <div className="text-lavender-300 text-3xl mt-4">❀</div>
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-2">{data.businessName}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-2 text-center">{data.notes}</p>}
    </div>
  );
}
...
function T16LineArt({ data, font, Icon, titleStyle, contentStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full bg-white flex flex-col px-[10%] py-[8%]" style={{ border: '1px solid #222' }}>
      ...
      <div style={{ width: '100%', height: 1, background: '#222' }} className="mt-4" />
      {data.businessName && <p style={{ fontFamily: font, fontSize: 12 * scale, color: data.contentColor }} className=" mt-3">{data.businessName}</p>}
      {data.serviceArea && <p style={{ fontFamily: font, fontSize: 11 * scale, color: data.contentColor, opacity: 0.7 }}>{data.serviceArea}</p>}
      {data.notes && <p style={{ fontFamily: font, fontSize: 10 * scale, color: data.contentColor, opacity: 0.7 }} className="/70 mt-2">{data.notes}</p>}
    </div>
  );
}

// T17: Soft Shadow
function T17SoftShadow({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
...
// T18: Neumorphism
function T18Neumorphism({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
...
// T19: Instagram Style
function T19InstagramStyle({ data, font, Icon, titleStyle, contentStyle, priceStyle, scale, services }: TProps) {
  return (
    <div className="w-full h-full flex flex-col px-[8%] py-[6%]" style={{ background: 'linear-gradient(135deg, #FB5EA8 0%, #CBB8FF 100%)' }}>
      <div className="bg-white rounded-3xl p-5 h-full flex flex-col">
        <div className="flex items-center gap-3 pb-3 border-b border-lavender-100">
          <Logo data={data} scale={scale} />
          <div>
            <p className="font-semibold" style={{ fontFamily: font, fontSize: 14 * scale, color: data.titleColor }}>{data.businessName || 'Your Business'}</p>
            <p style={{ fontFamily: font, fontSize: 11 * scale, color: data.contentColor, opacity: 0.7 }}>{data.serviceArea || 'Price List'}</p>
          </div>
        </div>
